...
public class CustomerController {
    ...
    public Response customerLogin(@QueryParam("userName") String userName, @QueryParam("password") String password) {
        try{
            if (CustomerFlipfitImplService.isUserValid(userName, password)) {
...
//        Customer customer=new Customer(userId,userName,email,password,customerPhone,cardDetails);
    ...
    public Response getCentresByCity(@QueryParam("city") String city){
        try{
            List<GymCentre> gymCentreList = CustomerFlipfitImplService.getAllGymCenterDetailsByCity(city);
    ...
    public Response getSlotsByCity(@QueryParam("centreId") String centreId,@QueryParam("date") String dateStr){
        try {
            ...
            List<Slot> slots =  CustomerFlipfitImplService.getAvailableSlots(centreId,sqlDate);
    ...
    public Response bookSlot(InputSlot slot){
        ...
        boolean slotBooked = CustomerFlipfitImplService.bookSlot(slot.getUserID(),slot.getDate(),slot.getSlotId(),slot.getCentreId());
    ...
    public Response cancelBooking(@QueryParam("bookingId") String bookingId){
        try {
            CustomerFlipfitServiceInterface CustomerFlipfitImplService = new CustomerFlipfitImplService();
            CustomerFlipfitImplService.cancelBookingbyID(bookingId);
