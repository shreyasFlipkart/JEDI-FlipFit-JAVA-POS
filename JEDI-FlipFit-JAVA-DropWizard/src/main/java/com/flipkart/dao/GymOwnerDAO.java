...
//        this.gymOwnerList = new ArrayList<>(gymOwnerList);
    ...
    public GymOwner loginGymOwnerWithObject(String username,String password) {
        ...
        try {
            ...
            ResultSet result;
            try {
                PreparedStatement statement = conn.prepareStatement(LOGIN_GYM_OWNER);
                ...
                while (result.next()) {
                    if (username.equals(result.getString("name")) && password.equals(result.getString("password"))) {
                        ...
                        gymOwner = new GymOwner(result.getString("Id"),result.getString("name"),result.getString("email"),result.getString("password"),result.getByte("isApproved"),result.getString("panNumber"),result.getString("cardDetails"));
                        return gymOwner;
                    } else {
                        return gymOwner;
                    }
    ...
    public GymOwner registerGymOwnerWithObject(GymOwner gymOwner) {
        try (Connection conn = DBUtils.connect();
             PreparedStatement stmt = conn.prepareStatement(REGISTER_GYM_OWNER)) {
            stmt.setString(1, gymOwner.getUserID());
            stmt.setString(2, gymOwner.getUserName());
